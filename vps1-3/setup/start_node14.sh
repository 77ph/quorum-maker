...
echo "[*] Starting Constellation node" > qdata/logs/node14.log

cp qdata/node14.conf .
...
constellation-node node14.conf 2>> qdata/logs/node14.log &
sleep 1

echo "[*] Starting node14 node" >> qdata/logs/constellation_node14.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT "  --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node14.log

PRIVATE_CONFIG=node14.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT   --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node14.log 
