...
echo "[*] Starting Constellation node" > qdata/logs/node13.log

cp qdata/node13.conf .
...
constellation-node node13.conf 2>> qdata/logs/node13.log &
sleep 1

echo "[*] Starting node13 node" >> qdata/logs/constellation_node13.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT " --voteaccount "0xa638f6bbe24aa90832cd7c581a8a869c3983e667" --votepassword "" --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node13.log

PRIVATE_CONFIG=node13.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT  --voteaccount "0xa638f6bbe24aa90832cd7c581a8a869c3983e667" --votepassword "" --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node13.log 
