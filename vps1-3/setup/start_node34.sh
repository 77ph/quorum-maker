...
echo "[*] Starting Constellation node" > qdata/logs/node34.log

cp qdata/node34.conf .
...
sed -i "$PATTERN" node34.conf
sed -i "$PATTERN2" node34.conf
sed -i "$PATTERN3" node34.conf
sed -i "$PATTERN4" node34.conf


constellation-node node34.conf 2>> qdata/logs/node34.log &
sleep 1

echo "[*] Starting node34 node" >> qdata/logs/constellation_node34.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT "  --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node34.log

PRIVATE_CONFIG=node34.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT   --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node34.log 
