...
echo "[*] Starting Constellation node" > qdata/logs/node35.log

cp qdata/node35.conf .
...
sed -i "$PATTERN" node35.conf
sed -i "$PATTERN2" node35.conf
sed -i "$PATTERN3" node35.conf
sed -i "$PATTERN4" node35.conf


constellation-node node35.conf 2>> qdata/logs/node35.log &
sleep 1

echo "[*] Starting node35 node" >> qdata/logs/constellation_node35.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT "  --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node35.log

PRIVATE_CONFIG=node35.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT   --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node35.log 
