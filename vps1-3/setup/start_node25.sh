...
echo "[*] Starting Constellation node" > qdata/logs/node25.log

cp qdata/node25.conf .
...
sed -i "$PATTERN" node25.conf
sed -i "$PATTERN2" node25.conf
sed -i "$PATTERN3" node25.conf
sed -i "$PATTERN4" node25.conf


constellation-node node25.conf 2>> qdata/logs/node25.log &
sleep 1

echo "[*] Starting node25 node" >> qdata/logs/constellation_node25.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT "  --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node25.log

PRIVATE_CONFIG=node25.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT   --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node25.log 
