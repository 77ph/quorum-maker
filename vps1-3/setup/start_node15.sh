...
echo "[*] Starting Constellation node" > qdata/logs/node15.log

cp qdata/node15.conf .
...
constellation-node node15.conf 2>> qdata/logs/node15.log &
sleep 1

echo "[*] Starting node15 node" >> qdata/logs/constellation_node15.log
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT "--port "$W_PORT "  --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/node15.log

PRIVATE_CONFIG=node15.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --port $W_PORT   --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/node15.log 
